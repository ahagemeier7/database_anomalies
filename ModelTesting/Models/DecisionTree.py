...
class DecisionTree:
  ...
  def _grow_tree(self,x,y,depth=0):
    ...
    #find the best split
    best_feature, best_tresh  = self._best_split(x,y,feat_idxs)

    #create child nodes
    left_idxs, right_idxs = self._split(x[:,best_feature], best_tresh)
    left = self._grow_tree(x[left_idxs,:], y[left_idxs], depth+1)
    right = self._grow_tree(x[right_idxs,:], y[right_idxs], depth+1)

    return Node(best_feature, best_tresh, left, right)
  ...
  def _information_gain(self,y,x_column,split_treshold):
    ...
    #create children
    left_idxs , right_idxs = self._split(x_column,split_treshold)
  ...
  def predict(self, x):
    return np.array([self._traverse_tree(xs, self.root) for xs in x])
