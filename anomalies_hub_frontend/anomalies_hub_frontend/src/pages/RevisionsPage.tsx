...
import { Calendar, Hash, Brain, AlertOctagon, CheckCircle, Search } from 'lucide-react';
...
export default function RevisionsPage() {
    ...
    fraudService.getAnomalies('pending_revision')
      .then(data => {
        const filtered = tableName
          ? data.filter((a: Anomaly) => a.origin_table === tableName)
          : data;
  ...
  useEffect(() => { fetchAnomalies(); }, [tableName]);
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`${anomalies.length} alert${anomalies.length !== 1 ? 's' : ''} awaiting review`}
      />
